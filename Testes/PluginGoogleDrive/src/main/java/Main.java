...
//		testDeleteFile();
	...
	public static Error tryCreateFolder(String folder) {		
		try {
			googleDrivePlugin.createFolder(folder);
	...
	public static Error tryListFolder(String folder) {
		try {
			ArrayList<String> files = googleDrivePlugin.listFolder(folder);
	...
	public static Error tryDeleteFolder(String folder) {		
		try {
			googleDrivePlugin.deleteFolder(folder);
		} catch(NullPointerException e) {
			return Error.NULL_POINTER;
		} catch(Exception e) {
			System.out.println(e);
	...
	public static Error tryCreateFile(String path) {		
		try {
			googleDrivePlugin.createFile(path);
		} catch(NullPointerException e) {
			return Error.NULL_POINTER;
	...
	public static Error tryReadFile(String path, byte[] contentExpected) {
		try {
			if(Arrays.equals(googleDrivePlugin.readFile(path), contentExpected) == false)
	...
	public static Error tryWriteFile(String path, byte[] content) {
		try {
			googleDrivePlugin.writeFile(path, content);
		} catch(NullPointerException e) {
			return Error.NULL_POINTER;
		} catch(NoSuchFileException e) {
			return Error.NO_SUCH_FILE;
	...
	public static Error tryDeleteFile(String path) {		
		try {
			googleDrivePlugin.deleteFile(path);
		} catch(NullPointerException e) {
			return Error.NULL_POINTER;
		} catch(NoSuchFileException e) {
			return Error.NO_SUCH_FILE;
